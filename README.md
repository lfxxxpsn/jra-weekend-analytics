...
- 依日期、競馬場瀏覽本週末出馬表。
- 依月份、日期與競馬場瀏覽 2026 年每場賽前條件預測，並以實際名次對照。
...
排程會在日本時間週四、週五及週末賽事時段執行。排程成功後，衍生統計、2026 預測分片與週末快照會由 GitHub Actions bot 寫回儲存庫，並主動觸發 Pages 部署。

## 資料設計

- `data/aggregate-store.json`：月份／賽事條件的彙總計數、已處理賽事 ID、活躍馬匹的精簡狀態。沒有保存 JRA 原始 HTML。
- `public/data/weekend.json`：本週出馬資料與條件排行。
- `public/data/predictions/2026/index.json`：2026 月份索引；`01.json` 至 `12.json` 按需載入逐場賽前預測，避免一次下載全年資料。
- `scripts/jra/`：低併發 HTTP、JRA POST 導航發現、Shift_JIS 解碼、頁面解析、統計與輸出。

歷史預測會按日期重播：先用該日開始前已知的統計與馬匹狀態產生排行，再套用當日賽果。實際名次只供對照，不參與原預測分數。bootstrap 的逐年 JSON 只作短期 artifact，正式儲存庫不保存原始 HTML。
